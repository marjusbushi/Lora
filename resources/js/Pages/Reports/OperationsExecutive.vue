...
const actionsTruncated = computed(() => Number(props.analytics.actions_truncated || 0));
...
const kpis = computed(() => [
    {
        label: translate('reports360.operationsExecutive.arrivals'),
        help: translate('reports360.help.opsArrivals'),
        value: `${number(flow.value.arrivals_remaining)} / ${number(flow.value.arrivals_total)}`,
        tone: Number(flow.value.arrivals_overdue) ? 'error' : Number(flow.value.arrivals_remaining) ? 'warning' : 'success',
        icon: LogIn,
        detail: `${number(flow.value.arrivals_completed)} ${translate('reports360.operationsExecutive.completed')}`,
        subtext: Number(flow.value.arrivals_overdue) ? overdueTag(flow.value.arrivals_overdue) : null,
    },
    {
        label: translate('reports360.operationsExecutive.departures'),
        help: translate('reports360.help.opsDepartures'),
        value: `${number(flow.value.departures_remaining)} / ${number(flow.value.departures_total)}`,
        tone: Number(flow.value.departures_overdue) ? 'error' : Number(flow.value.departures_remaining) ? 'warning' : 'success',
        icon: LogOut,
        detail: `${number(flow.value.departures_completed)} ${translate('reports360.operationsExecutive.completed')}`,
        subtext: Number(flow.value.departures_overdue) ? overdueTag(flow.value.departures_overdue) : null,
    },
    { label: translate('reports360.operationsExecutive.inHouse'), help: translate('reports360.help.opsInHouse'), value: number(flow.value.in_house_stays), tone: 'info', icon: Users, detail: `${number(flow.value.in_house_pax)} pax` },
    { label: translate('reports360.operationsExecutive.ready'), help: translate('reports360.help.opsReadyRate'), value: `${number(readiness.value.ready_rate, 1)}%`, tone: Number(readiness.value.ready_rate) >= 90 ? 'success' : 'warning', icon: CircleCheckBig, detail: `${number(readiness.value.ready_arrivals)} / ${number(readiness.value.arrivals_remaining)}` },
]);
...
const actionTitle = (action) => {
    ...
    if (action.kind === 'overdue_arrival' || action.kind === 'overdue_departure') return translate(`reports360.operationsExecutive.actions.${action.kind}`);
...
const actionDetail = (action) => {
    ...
    if (action.days_overdue > 0) parts.push(translate('reports360.operationsExecutive.daysOverdue', { n: number(action.days_overdue) }));
    if (action.balance > 0) parts.push(showBase.value ? `${money(action.balance)} (${moneyBase(action.balance)})` : money(action.balance));
    if (action.open_pos_count > 0) parts.push(`${action.open_pos_count} POS`);
    return parts.join(' · ') || '—';
};
const actionIcon = (action) => {
    if (action.kind === 'maintenance') return Wrench;
    if (action.kind === 'overdue_arrival') return LogIn;
    if (action.kind === 'overdue_departure') return LogOut;
...
<template>
    <ReportShell :title="$t('reports360.operationsExecutive.title')" :filters="null" :description="$t('reports360.operationsExecutive.short')" :category="$t('reports360.operationsExecutive.category')">
        <div class="mb-3 flex flex-wrap items-center justify-between gap-2 text-tiny text-neutral-500">
            <span v-if="displayRate">{{ $t('reports360.amountsShownIn', { currency: pricingCode }) }}</span>
            <span v-else />
            <span>{{ $t('reports360.operationsExecutive.updated') }} {{ updatedAt }}</span>
        </div>
        ...
        <Card class="mt-4" :padding="false">
            ...
            <div class="divide-y divide-neutral-100">
                <component :is="actionHref(action) ? Link : 'div'" v-for="action in actions" :key="action.key" v-bind="actionHref(action) ? { href: actionHref(action) } : {}" class="flex items-center gap-4 px-5 py-3 hover:bg-neutral-50">
                    <span class="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg" :class="action.severity === 'error' ? 'bg-error-50 text-error-600' : 'bg-warning-50 text-warning-700'"><component :is="actionIcon(action)" class="h-4 w-4" /></span>
                ...
                <div v-if="actionsTruncated > 0" class="px-5 py-3 text-center text-tiny text-neutral-500">{{ $t('reports360.operationsExecutive.moreActions', { n: actionsTruncated }) }}</div>
