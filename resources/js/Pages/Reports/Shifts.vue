...
const kpis = [
    ...
    { label: translate('admin.generated.k_e9c1e13432f8'), help: translate('reports360.help.zrCashSales'), value: () => money(props.totals?.cash), subvalue: () => showBase.value ? moneyBase(props.totals?.cash) : null, tone: 'success', icon: Banknote, href: shiftsHref() },
    { label: translate('admin.generated.k_a2daff3a669c'), value: () => money(props.totals?.card), subvalue: () => showBase.value ? moneyBase(props.totals?.card) : null, tone: 'info', icon: CreditCard, href: shiftsHref() },
    { label: translate('admin.generated.k_1d57e3177577'), help: translate('reports360.help.zrOverShort'), value: () => money(props.totals?.over_short), subvalue: () => showBase.value ? moneyBase(props.totals?.over_short) : null, detail: () => `${translate('reports360.zReport.pmsCash')}: ${money(props.totals?.pms_cash)}`, tone: () => Math.abs(Number(props.totals?.over_short ?? 0)) < 0.01 ? 'success' : 'error', icon: Scale, href: shiftsHref() },
];
</script>
                ...
                <table class="min-w-full divide-y divide-neutral-200">
                    <thead class="bg-neutral-50">
                        <tr>
                            ...
                            <th class="px-4 py-3 text-right text-label text-neutral-600"><span class="inline-flex items-center gap-1">{{ $t('admin.generated.k_620630b38207') }}<InfoTip :text="$t('reports360.help.zrExpectedCash')" :label="$t('admin.generated.k_620630b38207')" /></span></th>
                            <th class="px-4 py-3 text-right text-label text-neutral-600">{{ $t('admin.generated.k_0ec342ed75e4') }}</th>
                            <th class="px-4 py-3 text-right text-label text-neutral-600"><span class="inline-flex items-center gap-1">{{ $t('reports360.zReport.pmsCash') }}<InfoTip :text="$t('reports360.help.zrPmsCash')" :label="$t('reports360.zReport.pmsCash')" /></span></th>
                            <th class="px-4 py-3 text-right text-label text-neutral-600"><span class="inline-flex items-center gap-1">{{ $t('admin.generated.k_a912b90a105a') }}<InfoTip :text="$t('reports360.help.zrOverShort')" :label="$t('admin.generated.k_a912b90a105a')" /></span></th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-neutral-100">
                        <tr v-for="s in shifts" :key="s.id" class="hover:bg-neutral-50">
                            ...
                            <td class="px-4 py-3 text-right text-body-sm text-primary-900 font-medium">{{ money(s.counted_cash) }}<small v-if="showBase" class="block text-tiny font-normal text-neutral-400">{{ moneyBase(s.counted_cash) }}</small></td>
                            <td class="px-4 py-3 text-right text-body-sm text-neutral-600">{{ money(s.pms_cash) }}</td>
                            <td class="px-4 py-3 text-right whitespace-nowrap">
                                <Badge v-if="s.suspect_entry" variant="warning" size="sm" class="mr-1">{{ $t('reports360.zReport.suspectEntry') }}</Badge>
                    ...
                    <tfoot v-if="shifts.length" class="bg-neutral-50 border-t-2 border-neutral-200">
                        <tr>
                            ...
                            <td class="px-4 py-3 text-right text-body-sm font-semibold">{{ money(totals.pms_cash) }}</td>
