...
const staffName = (name) => name === 'Unassigned' ? translate('reports360.housekeepingProductivity.unassigned') : name;
const staffBars = computed(() => staff.value.map((row) => ({ key: row.staff, label: staffName(row.staff), value: row.completed, display: `${row.completed} · ${minutes(row.avg_clean_minutes)}`, barClass: 'bg-accent-500' })));
const kpis = computed(() => [
    { label: translate('reports360.housekeepingProductivity.completed'), help: translate('reports360.help.hkCompleted'), value: summary.value.completed || 0, tone: 'success', icon: CheckCircle2, detail: `${summary.value.total || 0} ${translate('reports360.housekeepingProductivity.assigned')}`, href: housekeepingHref() },
    { label: translate('reports360.housekeepingProductivity.completionRate'), help: translate('reports360.help.hkCompletionRate'), value: pct(summary.value.completion_rate), tone: 'accent', icon: Gauge, href: housekeepingHref() },
    { label: translate('reports360.housekeepingProductivity.avgClean'), help: translate('reports360.help.hkAvgClean'), value: minutes(summary.value.avg_clean_minutes), tone: 'info', icon: Sparkles, href: housekeepingHref() },
    { label: translate('reports360.housekeepingProductivity.avgQueue'), help: translate('reports360.help.hkAvgQueue'), value: minutes(summary.value.avg_queue_minutes), tone: 'warning', icon: Clock3, detail: `${summary.value.issues || 0} ${translate('reports360.housekeepingProductivity.issues')}`, href: housekeepingHref() },
]);
</script>
...
<template>
    <ReportShell :title="$t('reports360.housekeepingProductivity.title')" route-name="reports.housekeepingReport" :filters="filters" :description="$t('reports360.housekeepingProductivity.short')" :category="$t('reports360.housekeepingProductivity.category')">
        ...
        <Card class="mt-4" :padding="false">
            ...
            <div class="overflow-x-auto"><table class="min-w-full divide-y divide-neutral-200"><thead class="bg-neutral-50 text-label text-neutral-600"><tr><th class="px-5 py-3 text-left">{{ $t('reports360.housekeepingProductivity.staff') }}</th><th class="px-4 py-3 text-right">{{ $t('reports360.housekeepingProductivity.assigned') }}</th><th class="px-4 py-3 text-right">{{ $t('reports360.housekeepingProductivity.completed') }}</th><th class="px-4 py-3 text-right"><span class="inline-flex items-center gap-1">{{ $t('reports360.housekeepingProductivity.completionRate') }}<InfoTip :text="$t('reports360.help.hkCompletionRate')" :label="$t('reports360.housekeepingProductivity.completionRate')" /></span></th><th class="px-4 py-3 text-right">{{ $t('reports360.housekeepingProductivity.avgClean') }}</th><th class="px-5 py-3 text-right"><span class="inline-flex items-center gap-1">{{ $t('reports360.housekeepingProductivity.issues') }}<InfoTip :text="$t('reports360.help.hkIssues')" :label="$t('reports360.housekeepingProductivity.issues')" /></span></th></tr></thead>
                <tbody class="divide-y divide-neutral-100"><tr v-for="row in staff" :key="row.staff" class="hover:bg-neutral-50"><td class="px-5 py-3 text-body-sm font-medium text-primary-900">{{ staffName(row.staff) }}</td><td class="px-4 py-3 text-right text-body-sm text-neutral-600">{{ row.assigned }}</td><td class="px-4 py-3 text-right text-body-sm font-semibold text-success-700">{{ row.completed }}</td><td class="px-4 py-3 text-right text-body-sm text-neutral-700">{{ pct(row.completion_rate) }}</td><td class="px-4 py-3 text-right text-body-sm text-neutral-700">{{ minutes(row.avg_clean_minutes) }}</td><td class="px-5 py-3 text-right text-body-sm" :class="row.issues ? 'text-error-700' : 'text-neutral-500'">{{ row.issues }}</td></tr></tbody></table></div>
        </Card>
        ...
        <Card class="mt-4" :padding="false">
            ...
            <div class="overflow-x-auto"><table class="min-w-full divide-y divide-neutral-200"><thead class="bg-neutral-50 text-label text-neutral-600"><tr><th class="px-5 py-3 text-left">{{ $t('reports360.housekeepingProductivity.room') }}</th><th class="px-4 py-3 text-left">{{ $t('reports360.housekeepingProductivity.type') }}</th><th class="px-4 py-3 text-left">{{ $t('reports360.housekeepingProductivity.staff') }}</th><th class="px-4 py-3 text-left">{{ $t('reports360.housekeepingProductivity.status') }}</th><th class="px-4 py-3 text-right"><span class="inline-flex items-center gap-1">{{ $t('reports360.housekeepingProductivity.queue') }}<InfoTip :text="$t('reports360.help.hkAvgQueue')" :label="$t('reports360.housekeepingProductivity.queue')" /></span></th><th class="px-4 py-3 text-right"><span class="inline-flex items-center gap-1">{{ $t('reports360.housekeepingProductivity.cleaning') }}<InfoTip :text="$t('reports360.help.hkAvgClean')" :label="$t('reports360.housekeepingProductivity.cleaning')" /></span></th><th class="px-5 py-3 text-right">{{ $t('reports360.housekeepingProductivity.date') }}</th></tr></thead>
            ...
            <div v-if="analytics.tasks_truncated > 0" class="px-5 py-3 text-center text-tiny text-neutral-500">{{ $t('reports360.moreRows', { n: analytics.tasks_truncated }) }}</div>
