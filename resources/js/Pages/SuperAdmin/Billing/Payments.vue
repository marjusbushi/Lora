...
const cards = computed(() => [
    { label: translate('superAdmin.billing.collectedThisMonth'), value: money(props.stats.month_cents), detail: translate('superAdmin.billing.allMethods'), icon: WalletCards },
    { label: translate('superAdmin.billing.manualPayments'), value: money(props.stats.manual_cents), detail: translate('superAdmin.billing.bankCashOther'), icon: Landmark },
    { label: translate('superAdmin.billing.onlinePayments'), value: money(props.stats.online_cents), detail: translate('superAdmin.billing.integratedProviders'), icon: CreditCard },
]);
...
function methodLabel(method) {
    return { bank_transfer: translate('superAdmin.billing.methodBankTransfer'), cash: 'Cash', other: translate('superAdmin.billing.methodOther') }[method] || method;
}
...
<template>
    <SuperAdminLayout :title="$t('superAdmin.billing.paymentsPageTitle')">
        <main class="sa-page max-w-[1320px] space-y-4">
            <BillingPageHeader :title="$t('superAdmin.compact.payments')">
                <template #actions><button type="button" class="sa-button sa-button-primary" @click="modalOpen = true"><Banknote /> {{ $t('superAdmin.billing.registerPayment') }}</button></template>
            </BillingPageHeader>
            ...
            <section class="sa-card">
                <div class="sa-card-header flex-col items-stretch md:flex-row md:items-end"><div><h2 class="sa-card-title">{{ $t('superAdmin.billing.transactions') }}</h2></div><div class="flex flex-wrap gap-2"><label>{{ $t('superAdmin.compact.hotel') }}<select :value="filters.tenant_id || ''" class="sa-control mt-1 block min-w-[160px]" @change="filter('tenant_id', $event.target.value)"><option value="">{{ $t('superAdmin.compact.all') }}</option><option v-for="tenant in tenants" :key="tenant.id" :value="tenant.id">{{ tenant.name }}</option></select></label><label>{{ $t('superAdmin.compact.status') }}<select :value="filters.status || ''" class="sa-control mt-1 block min-w-[130px]" @change="filter('status', $event.target.value)"><option value="">{{ $t('superAdmin.activity.all') }}</option><option value="completed">{{ $t('superAdmin.billing.statusCompletedShort') }}</option><option value="pending">{{ $t('superAdmin.billing.statusInProcess') }}</option><option value="failed">{{ $t('superAdmin.billing.statusFailed') }}</option><option value="refunded">{{ $t('superAdmin.billing.statusRefunded') }}</option></select></label></div></div>
                <div class="overflow-x-auto"><table class="min-w-full text-left"><thead><tr class="sa-table-head"><th class="px-4 py-2.5 font-bold">{{ $t('superAdmin.billing.payment') }}</th><th class="px-4 py-2.5 font-bold">{{ $t('superAdmin.compact.hotel') }}</th><th class="px-4 py-2.5 font-bold">{{ $t('superAdmin.billing.invoice') }}</th><th class="px-4 py-2.5 font-bold">{{ $t('superAdmin.billing.method') }}</th><th class="px-4 py-2.5 font-bold">{{ $t('superAdmin.activity.date') }}</th><th class="px-4 py-2.5 font-bold">{{ $t('superAdmin.compact.status') }}</th><th class="px-4 py-2.5 text-right font-bold">{{ $t('superAdmin.billing.amount') }}</th></tr></thead><tbody class="divide-y divide-neutral-100"><tr v-for="payment in payments.data" :key="payment.id" class="hover:bg-neutral-50"><td class="px-4 py-3"><Link :href="`/super-admin/billing/payments/${payment.id}`" class="sa-table-primary text-emerald-700 no-underline">{{ payment.number }}</Link></td><td class="px-4 py-3"><Link :href="`/super-admin/tenants/${payment.tenant.id}`" class="sa-table-primary no-underline hover:text-emerald-700">{{ payment.tenant.name }}</Link></td><td class="px-4 py-3"><Link v-if="payment.invoice" :href="`/super-admin/billing/invoices/${payment.invoice.id}`" class="text-xs font-semibold text-emerald-700 no-underline">{{ payment.invoice.number }}</Link><span v-else>—</span></td><td class="px-4 py-3 text-xs text-neutral-500"><span class="block">{{ payment.provider === 'manual' ? methodLabel(payment.method) : payment.provider }}</span><span class="sa-table-meta">{{ payment.reference || $t('superAdmin.billing.noReference') }}</span></td><td class="whitespace-nowrap px-4 py-3 text-xs text-neutral-500">{{ dateTime(payment.paid_at) }}</td><td class="px-4 py-3"><span class="rounded-full bg-emerald-50 px-2 py-1 text-[10px] font-bold text-emerald-700">{{ payment.status }}</span></td><td class="whitespace-nowrap px-4 py-3 text-right text-xs font-semibold text-neutral-900">{{ money(payment.amount_cents, payment.currency) }}</td></tr><tr v-if="!payments.data.length"><td colspan="7" class="px-4 py-10 text-center text-xs text-neutral-500">{{ $t('superAdmin.billing.noPaymentsYet') }}</td></tr></tbody></table></div>
        ...
        <Teleport to="body">
            <div v-if="modalOpen" class="fixed inset-0 z-50 flex items-end justify-center bg-neutral-950/50 p-0 sm:items-center sm:p-6" @click.self="modalOpen = false">
                <form class="max-h-[94vh] w-full max-w-2xl overflow-y-auto rounded-t-2xl bg-white shadow-2xl sm:rounded-2xl" @submit.prevent="submit">
                    <div class="flex items-start justify-between border-b border-neutral-200 px-5 py-4"><div><h2 class="text-lg font-semibold text-neutral-900">{{ $t('superAdmin.billing.registerManualPayment') }}</h2><p class="mt-1 text-sm text-neutral-500">{{ $t('superAdmin.billing.manualPaymentSubtitle') }}</p></div><button type="button" class="rounded-lg p-2 text-neutral-400 hover:bg-neutral-100" @click="modalOpen = false"><X class="h-5 w-5" /></button></div>
                    <div class="grid gap-4 p-5 sm:grid-cols-2">
                        <label class="text-sm font-medium text-neutral-700 sm:col-span-2">{{ $t('superAdmin.billing.invoice') }}<select v-model="form.billing_invoice_id" required class="mt-1 w-full rounded-xl border-neutral-300 text-sm"><option value="" disabled>{{ $t('superAdmin.billing.chooseInvoice') }}</option><option v-for="invoice in openInvoices" :key="invoice.id" :value="invoice.id">{{ invoice.number }} · {{ invoice.tenant_name }} · {{ money(invoice.balance_cents, invoice.currency) }}</option></select><span v-if="form.errors.billing_invoice_id" class="mt-1 block text-xs text-red-600">{{ form.errors.billing_invoice_id }}</span></label>
                    ...
                    <div class="flex justify-end gap-3 border-t border-neutral-200 px-5 py-4"><button type="button" class="rounded-xl border border-neutral-300 px-4 py-2 text-sm font-semibold" @click="modalOpen = false">{{ $t('superAdmin.auto.copy008') }}</button><button class="rounded-xl bg-[#16875d] px-4 py-2 text-sm font-semibold text-white" :disabled="form.processing">{{ form.processing ? $t('superAdmin.billing.registering') : $t('superAdmin.billing.confirmPayment') }}</button></div>
                </form>
            </div>
        </Teleport>
    </SuperAdminLayout>
