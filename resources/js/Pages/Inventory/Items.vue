...
const writeOffReasons = { damaged: translate('inventoryItems.reasons.damaged'), lost: translate('inventoryItems.reasons.lost'), expired: translate('inventoryItems.reasons.expired'), other: translate('inventoryItems.reasons.other') };
...
function deleteCategory(category) {
    if (!confirm(translate('inventoryItems.deleteCategoryConfirm', { name: category.name }))) return;
...
<template>
    <AppLayout>
        ...
        <Modal :show="!!editing" :title="editing === 'new' ? $t('inventoryItems.newProduct') : $t('inventoryItems.editProduct')" max-width="4xl" @close="closeModal">
            <div class="space-y-5">
                <section class="rounded-xl border border-neutral-200 p-4">
                    <div class="mb-4 flex items-center gap-2"><Package class="h-5 w-5 text-accent-700" /><div><h4 class="font-bold text-primary-900">{{ $t('inventoryItems.productData') }}</h4><p class="text-tiny text-neutral-500">{{ $t('inventoryItems.photoHint') }}</p></div></div>
                    <div class="grid gap-5 md:grid-cols-[180px_1fr]">
                        <div>
                            <button type="button" class="group relative grid aspect-square w-full place-items-center overflow-hidden rounded-xl border-2 border-dashed border-neutral-200 bg-neutral-50 hover:border-accent-400" @click="fileInput?.click()">
                                <img v-if="imagePreview" :src="imagePreview" class="h-full w-full object-cover" :alt="$t('inventoryItems.productImageAlt')" />
                                <span v-else class="flex flex-col items-center gap-2 text-center text-neutral-400"><ImagePlus class="h-7 w-7" /><span class="text-tiny font-semibold">{{ $t('inventoryItems.uploadPhoto') }}<br />{{ $t('inventoryItems.photoFormats') }}</span></span>
                            </button>
                            <input ref="fileInput" type="file" accept="image/jpeg,image/png,image/webp" class="hidden" @change="selectImage" />
                            <button v-if="imagePreview" type="button" class="mt-2 flex w-full items-center justify-center gap-1 text-tiny font-semibold text-error-600" @click="removeImage"><Trash2 class="h-3.5 w-3.5" /> {{ $t('inventoryItems.removePhoto') }}</button>
                            <p v-if="form.errors.image" class="mt-1 text-tiny text-error-600">{{ form.errors.image }}</p>
                        </div>
                        <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                            <div class="sm:col-span-2"><label class="mb-1 block text-body-sm font-semibold">{{ $t('inventoryItems.productName') }}</label><TextInput v-model="form.name" class="w-full" :placeholder="$t('inventoryItems.productNamePlaceholder')" /><p v-if="form.errors.name" class="mt-1 text-tiny text-error-600">{{ form.errors.name }}</p></div>
                            ...
                            <div><label class="mb-1 block text-body-sm font-semibold">{{ $t('inventoryItems.category') }}</label><select v-model="form.category_id" class="w-full rounded-lg border-neutral-200 px-3 py-2 text-body-sm"><option :value="null">{{ $t('inventory.categories.none') }}</option><option v-for="category in categories" :key="category.id" :value="category.id">{{ categoryLabel(category) }}</option></select><p v-if="form.errors.category_id" class="mt-1 text-tiny text-error-600">{{ form.errors.category_id }}</p></div>
                            <div><label class="mb-1 block text-body-sm font-semibold">{{ $t('inventoryItems.type') }}</label><select v-model="form.type" class="w-full rounded-lg border-neutral-200 px-3 py-2 text-body-sm"><option v-for="(label, key) in typeLabels" :key="key" :value="key">{{ label }}</option></select></div>
                            <div v-if="form.type !== 'service'"><label class="mb-1 block text-body-sm font-semibold">{{ $t('inventoryItems.unit') }}</label><select v-model="form.unit" class="w-full rounded-lg border-neutral-200 px-3 py-2 text-body-sm"><option v-for="(label, key) in unitLabels" :key="key" :value="key">{{ label }}</option></select></div>
                            <div v-if="form.type !== 'service'"><label class="mb-1 block text-body-sm font-semibold">{{ $t('inventoryItems.minimumStock') }}</label><TextInput v-model="form.minimum_stock" type="number" min="0" step="0.0001" class="w-full" /></div>
                            <label v-if="editing !== 'new'" class="flex items-center gap-2 self-end pb-2 text-body-sm font-semibold"><input v-model="form.is_active" type="checkbox" class="rounded border-neutral-300 text-accent-600 focus:ring-accent-500" /> {{ $t('inventoryItems.activeProduct') }}</label>
                        </div>
                    </div>
                </section>
                ...
                <section v-if="form.type === 'product'" class="grid gap-4 md:grid-cols-2">
                    <div class="rounded-xl border p-4" :class="form.sell_in_pos ? 'border-blue-200 bg-blue-50/40' : 'border-neutral-200'">
                        <label class="flex cursor-pointer items-start justify-between gap-4"><span class="flex gap-3"><span class="grid h-10 w-10 place-items-center rounded-lg bg-blue-100 text-blue-700"><ShoppingBasket class="h-5 w-5" /></span><span><strong class="block text-primary-900">{{ $t('inventoryItems.sellInPos') }}</strong><small class="text-neutral-500">{{ $t('inventoryItems.sellInPosHint') }}</small></span></span><input v-model="form.sell_in_pos" type="checkbox" class="mt-2 rounded border-neutral-300 text-accent-600 focus:ring-accent-500" /></label>
                        <div v-if="form.sell_in_pos" class="mt-4 grid gap-3 sm:grid-cols-2">
                            <div><label class="mb-1 block text-tiny font-bold">{{ $t('inventoryItems.posWarehouse') }}</label><select v-model="form.pos_warehouse_id" class="w-full rounded-lg border-neutral-200 px-3 py-2 text-body-sm"><option v-for="warehouse in warehouses" :key="warehouse.id" :value="warehouse.id">{{ warehouse.name }}</option></select><p v-if="form.errors.pos_warehouse_id" class="mt-1 text-tiny text-error-600">{{ form.errors.pos_warehouse_id }}</p></div>
                            <div><label class="mb-1 block text-tiny font-bold">{{ $t('inventoryItems.posPrice') }} ({{ currencySymbol }})</label><TextInput v-model="form.selling_price" type="number" min="0.01" step="0.01" class="w-full" /><p v-if="form.errors.selling_price" class="mt-1 text-tiny text-error-600">{{ form.errors.selling_price }}</p></div>
                            <p class="sm:col-span-2 rounded-lg bg-blue-50 px-3 py-2 text-tiny text-blue-800">{{ $t('inventoryItems.posCategoryNote') }}</p>
                        </div>
                    </div>
                    <div class="rounded-xl border p-4" :class="form.sell_in_rooms ? 'border-violet-200 bg-violet-50/40' : 'border-neutral-200'">
                        <label class="flex cursor-pointer items-start justify-between gap-4"><span class="flex gap-3"><span class="grid h-10 w-10 place-items-center rounded-lg bg-violet-100 text-violet-700"><BedDouble class="h-5 w-5" /></span><span><strong class="block text-primary-900">{{ $t('inventoryItems.sellInRooms') }}</strong><small class="text-neutral-500">{{ $t('inventoryItems.sellInRoomsHint') }}</small></span></span><input v-model="form.sell_in_rooms" type="checkbox" class="mt-2 rounded border-neutral-300 text-accent-600 focus:ring-accent-500" /></label>
                        <div v-if="form.sell_in_rooms" class="mt-4 grid gap-3 sm:grid-cols-2">
                            <div><label class="mb-1 block text-tiny font-bold">{{ $t('inventoryItems.roomsWarehouse') }}</label><select v-model="form.room_warehouse_id" class="w-full rounded-lg border-neutral-200 px-3 py-2 text-body-sm"><option v-for="warehouse in warehouses" :key="warehouse.id" :value="warehouse.id">{{ warehouse.name }}</option></select><p v-if="form.errors.room_warehouse_id" class="mt-1 text-tiny text-error-600">{{ form.errors.room_warehouse_id }}</p></div>
                            <div><label class="mb-1 block text-tiny font-bold">{{ $t('inventoryItems.roomPrice') }} ({{ currencySymbol }})</label><TextInput v-model="form.room_selling_price" type="number" min="0.01" step="0.01" class="w-full" /><p v-if="form.errors.room_selling_price" class="mt-1 text-tiny text-error-600">{{ form.errors.room_selling_price }}</p></div>
                        </div>
                    </div>
                </section>

                <section v-if="editing === 'new' && form.type !== 'service'" class="rounded-xl border border-accent-200 bg-accent-50/50 p-4"><h4 class="text-body-sm font-bold text-primary-900">{{ $t('inventoryItems.initialStock') }}</h4><p class="mt-1 text-tiny text-neutral-500">{{ $t('inventoryItems.initialStockHint') }}</p><div class="mt-3 grid gap-3 sm:grid-cols-3"><div><label class="mb-1 block text-tiny font-semibold">{{ $t('inventoryItems.quantity') }}</label><TextInput v-model="form.initial_quantity" type="number" min="0" step="0.0001" class="w-full" /></div><div><label class="mb-1 block text-tiny font-semibold">{{ $t('inventoryItems.costPerUnit') }} ({{ currencySymbol }})</label><TextInput v-model="form.average_cost" type="number" min="0" step="0.01" class="w-full" /></div><div><label class="mb-1 block text-tiny font-semibold">{{ $t('inventoryItems.warehouse') }}</label><select v-model="form.initial_warehouse_id" class="w-full rounded-lg border-neutral-200 px-3 py-2 text-body-sm"><option v-for="warehouse in warehouses" :key="warehouse.id" :value="warehouse.id">{{ warehouse.name }}</option></select></div></div></section>
            </div>
