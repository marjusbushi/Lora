...
const typeLabels = { central: 'Qendrore', bar: 'Bar', restaurant: 'Restorant', rooms: 'Dhoma', housekeeping: 'Mirëmbajtje', other: 'Tjetër' };
        ...
        <div class="mx-auto max-w-[1500px] space-y-6">
            ...
            <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                <Card v-for="warehouse in warehouses" :key="warehouse.id">
                    ...
                    <div class="mt-4 grid grid-cols-3 divide-x divide-neutral-100 rounded-lg bg-neutral-50 py-3 text-center"><div><strong class="block text-body-sm text-primary-900">{{ warehouse.items_count }}</strong><span class="text-tiny text-neutral-400">artikuj</span></div><div><strong class="block text-body-sm text-primary-900">{{ money(warehouse.stock_value) }}</strong><span class="text-tiny text-neutral-400">vlerë</span></div><div><strong class="block text-body-sm" :class="warehouse.low_stock_count ? 'text-warning-700' : 'text-primary-900'">{{ warehouse.low_stock_count }}</strong><span class="text-tiny text-neutral-400">stok ulët</span></div></div>
                </Card>
            </div>
