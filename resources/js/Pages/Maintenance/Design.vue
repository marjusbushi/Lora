...
function submitReport() {
    // '__manual' is the UI sentinel for "Tjetër" — the server expects an
    // empty issue_key + a manual title for free-text reports.
    reportForm.transform((data) => ({ ...data, issue_key: data.issue_key === '__manual' ? '' : data.issue_key })).post(route('maintenance.store'), {
        forceFormData: true,
        preserveScroll: true,
