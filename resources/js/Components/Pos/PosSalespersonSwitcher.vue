...
function submit() {
    ...
    router.post(target, { user_id: selected.value.id, pin: pin.value }, {
        ...
        onError: (errors) => { error.value = errors.pin || errors.user_id || translate('shared.posSalesperson.switchError'); pin.value = ''; },
...
<template>
    <button v-if="compact" type="button" class="rounded-lg border border-neutral-200 bg-white px-3 py-2 text-small font-semibold text-accent-700 transition hover:border-accent-300 hover:bg-accent-50" @click="open">{{ $t('shared.posSalesperson.change') }}</button>
    <button v-else type="button" class="flex shrink-0 items-center rounded-xl border border-neutral-200 bg-white text-left shadow-card transition hover:border-accent-300 hover:bg-accent-50" :class="dense ? 'h-12 min-w-40 gap-2 px-3' : 'h-14 min-w-44 gap-3 px-3.5'" @click="open">
        ...
        <span class="min-w-0 flex-1"><span class="block text-tiny font-semibold uppercase tracking-wide text-neutral-400">Salesperson</span><strong class="block truncate text-body-sm text-primary-900">{{ current?.name || $t('shared.posSalesperson.select') }}</strong></span>
        <ChevronDown class="h-4 w-4 shrink-0 text-neutral-400" />
    </button>

    <Modal :show="show" :title="orderId ? $t('shared.posSalesperson.transferTitle') : $t('shared.posSalesperson.switchTitle')" max-width="sm" @close="show = false">
        <template v-if="!selected">
            <p class="mb-3 text-body-sm text-neutral-500">{{ $t('shared.posSalesperson.chooseStaff') }}</p>
            <div class="grid gap-2">
                <button v-for="person in salespeople" :key="person.id" type="button" class="flex items-center gap-3 rounded-xl border border-neutral-200 p-3 text-left transition" :class="person.has_pin ? 'hover:border-accent-300 hover:bg-accent-50' : 'cursor-not-allowed opacity-50'" @click="choose(person)">
                    ...
                    <span class="flex-1"><strong class="block text-body-sm text-primary-900">{{ person.name }}</strong><span class="text-tiny text-neutral-500">{{ person.has_pin ? $t('shared.posSalesperson.pinActive') : $t('shared.posSalesperson.setPinHint') }}</span></span>
                    <span v-if="Number(person.id) === Number(current?.id)" class="rounded-full bg-success-50 px-2 py-1 text-tiny font-semibold text-success-700">{{ $t('shared.posSalesperson.active') }}</span>
                </button>
            </div>
        </template>
        <template v-else>
            <div class="text-center"><p class="text-body-sm text-neutral-500">{{ $t('shared.posSalesperson.pinFor') }}</p><p class="mt-1 text-h4 text-primary-900">{{ selected.name }}</p></div>
            ...
            <div class="mx-auto grid max-w-[250px] grid-cols-3 gap-2">
                ...
                <button type="button" class="h-14 rounded-xl text-small text-neutral-500 hover:bg-neutral-50" @click="selectedId = null">{{ $t('shared.posSalesperson.staff') }}</button>
                ...
                <button type="button" class="h-14 rounded-xl text-small text-neutral-500 hover:bg-neutral-50" @click="pin = pin.slice(0, -1)">{{ $t('shared.posSalesperson.deleteDigit') }}</button>
            </div>
        </template>
        <template #footer><Button variant="ghost" @click="show = false">{{ $t('shared.posSalesperson.cancel') }}</Button></template>
    </Modal>
</template>
