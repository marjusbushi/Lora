...
return new class extends Migration
{
    ...
    {
        DB::table('tenant_module_entitlements')->where('module_code', 'beach')->delete();

        DB::table('tenants')->orderBy('id')->each(function (object $tenant) {
            $metadata = json_decode((string) ($tenant->metadata ?? '{}'), true);
            $metadata = is_array($metadata) ? $metadata : [];
            unset($metadata['billing_access']['modules']['beach']);
