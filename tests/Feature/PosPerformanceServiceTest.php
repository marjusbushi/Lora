...
class PosPerformanceServiceTest extends TestCase
{
    ...
    public function test_it_combines_sales_hourly_demand_items_and_margin(): void
    {
        ...
        $this->order($user, $item, '2026-07-09', '2026-07-09 11:00:00', 60, 10, 50, 1);

        $report = app(PosPerformanceService::class)
            ->withComparison(new ReportingPeriod('2026-07-10', '2026-07-10'));
        $current = $report['current'];

        $this->assertSame(100.0, $current['summary']['total_revenue']);
        $this->assertSame(1, $current['summary']['order_count']);
        $this->assertSame(100.0, $current['summary']['avg_ticket']);
        $this->assertSame(40.0, $current['summary']['estimated_cost']);
        $this->assertSame(60.0, $current['summary']['gross_profit']);
        $this->assertSame(60.0, $current['summary']['gross_margin']);
        $this->assertSame(100.0, collect($current['hours'])->firstWhere('hour', 13)['revenue']);
        $this->assertSame(100.0, $current['categories'][0]['revenue']);
        $this->assertSame('Pasta', $current['top_items'][0]['name']);
        $this->assertSame(100.0, $report['changes']['revenue']);
        $this->assertSame(0.0, $report['changes']['gross_margin']);
    }
